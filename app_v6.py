...
with tab2:
    ...
    with col1:
        ...
        if metric not in df_comparison.columns:
            ...
            if level == "State":
                state_level_count["Name"] = state_level_count["State"]
                top_df = state_level_count.sort_values(target_col, ascending=False).head(top_n)
                y_col='Name'
                hover_cols = ["State", target_col]
            if level == "County":
                county_level_count["Name"] = county_level_count["County"] + ", " + county_level_count["State"]
                top_df = county_level_count.sort_values(target_col, ascending=False).head(top_n)
                y_col='Name'
                hover_cols = ["State", target_col]
        else:
            target_col = metric 
...
with tab3:
    ...
    with col1:
        ...
        # Initialize top_df again since top_df could be food_desert_count.csv if the user selects a food desert count metric 
        top_df = df_comparison.sort_values(target_col, ascending=False).round(2).head(top_n)
